...
const Button = ({text, isOutline, onClick}:Props) => {
  return (
    <div>
        ...
        <div onClick={onClick} className='text-[0.9rem] p-[0.5rem] w-[100%] hover:scale-110 duration-500   cursor-pointer pr-3 pl-3 flex flex-row min-w-[3rem] items-center justify-center text-black rounded-[0.1rem] bg-[#d44be0] font-semibold'>{text}</div>
        }
    </div>
  )
}
